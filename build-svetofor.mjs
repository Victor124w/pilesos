...
// Матрица широкая (столбец на каждый месяц/день) — закрепляем не только шапку, но и
// 3 первые колонки: иначе при скролле вправо не видно, чья это строка. В svetofor.mjs
// стоит ws['!freeze']={xSplit:3,ySplit:1}, но SheetJS его игнорирует — работает только это.
const out = Buffer.from(freezePane(new Uint8Array(buf), 3, 1));
